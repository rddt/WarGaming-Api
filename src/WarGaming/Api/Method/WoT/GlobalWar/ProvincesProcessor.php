<?php

/**
 * This file is part of the WarGaming API package
 *
 * (c) Vitaliy Zhuk
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code
 */

namespace WarGaming\Api\Method\WoT\GlobalWar;

use Guzzle\Http\Message\Response as GuzzleResponse;
use WarGaming\Api\Method\AbstractProcessor;
use WarGaming\Api\Method\MethodInterface;
use WarGaming\Api\Model\WoT\Province;
use WarGaming\Api\Model\WoT\ProvinceCollection;

/**
 * Get provinces API processor
 *
 * @author Mike Flisher <mike.flisher@gmail.com>
 */
class ProvincesProcessor extends AbstractProcessor
{
    /**
     * {@inheritDoc}
     */
    protected function getApiUri()
    {
        return 'wot/globalwar/provinces';
    }

    /**
     * {@inheritDoc}
     */
    public function parseResponse(array $data, array $fullData, GuzzleResponse $response, MethodInterface $method)
    {
        $provinces = new ProvinceCollection();

        foreach ($data as $info) {
            $province = Province::createFromArray($info);
            $province->setFullDataFromArray($info);
            $provinces[$province->id] = $province;
        }

        return $provinces;

    }
}
